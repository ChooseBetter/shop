<template>
  <div class="cart">
    ...
    <div class="layer" v-if="validTotal && $route.path !== '/cart'">
      <div class="list">
        <div class="item" v-for="goods in validList" :key="goods.skuId">
...
const {validList, validTotal, validAmount} = storeToRefs(cartStore);

cartStore.findCart().then((msg) => {
  Message({type: "success", text: msg});
});
const deleteCart = (skuId: string) => {
  cartStore.deleteCart(skuId).then((msg) => {
    Message({type: "success", text: msg});
  });
};
</script>
